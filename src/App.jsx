...
export default function App(){
  ...
  {mv==="panel"&&<div>
    ...
    <div style={B}>{(function(){var ls=cl;if(fs!=="todos")ls=ls.filter(function(c){return c.status===fs;});if(sr.trim())ls=ls.filter(function(c){return c.name&&c.name.toLowerCase().indexOf(sr.toLowerCase())>=0;});if(!ls.length)return<div style={{padding:40,textAlign:"center",color:"#475569"}}>Sin clientes</div>;return ls.map(function(c){var s=ST[c.status]||ST.activo;return<div key={c.id} onClick={function(){setSel(c);setTab("perfil");setMv("clientes");}} style={{padding:"16px 20px",borderBottom:"1px solid #1e2330",display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}><div style={{width:36,height:36,borderRadius:9,background:s.c+"15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:s.c}}>{ini(c)}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</div><div style={{fontSize:10,color:"#64748b"}}>{c.objective}</div></div><select value={c.status} onClick={function(e){e.stopPropagation();}} onChange={function(e){var v=e.target.value;sv(function(p){return p.map(function(x){return x.id===c.id?Object.assign({},x,{status:v}):x;});});}} style={{padding:"4px 8px",background:s.c+"10",border:"1px solid "+s.c+"25",borderRadius:7,color:s.c,fontSize:10,fontWeight:600,outline:"none"}}>{Object.entries(ST).map(function(e){return<option key={e[0]} value={e[0]}>{e[1].l}</option>;})}</select><button onClick={function(e){e.stopPropagation();if(confirm("¿Eliminar DEFINITIVAMENTE a "+c.name+"?")){setCl(function(p){return p.filter(function(x){return x.id!==c.id;});});deleteClient(c.id);if(sel&&sel.id===c.id)setSel(null);}}} style={{padding:"6px 10px",background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",borderRadius:7,color:"#ef4444",fontSize:11,cursor:"pointer",fontWeight:600,flexShrink:0}}>🗑️</button></div>;});})()}</div>
  </div>}
