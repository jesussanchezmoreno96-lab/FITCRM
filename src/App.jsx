...
export default function App(){
  ...
  {mv==="leads"&&<div>
    ...
    <div style={B}>{(function(){var ls=le.slice();if(lfl==="activos")ls=ls.filter(function(l){return l.status!=="convertido"&&l.status!=="perdido";});else ls=ls.filter(function(l){return l.status===lfl;});if(!ls.length)return<div style={{padding:40,textAlign:"center",color:"#475569"}}>{le.length===0?"Añade tu primer lead":"Sin leads"}</div>;return ls.map(function(l){var st=LS.find(function(x){return x.v===l.status;})||LS[0];return<div key={l.id} style={{padding:"12px 18px",borderBottom:"1px solid #1e2330",display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18}}>{st.i}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700}}>{l.name}{l.source?<span style={{fontSize:9,padding:"2px 5px",borderRadius:5,background:"#1e2330",color:"#94a3b8",marginLeft:6}}>{l.source}</span>:null}</div>{l.interest&&<div style={{fontSize:10,color:"#64748b"}}>{l.interest}</div>}</div><select value={l.status} onChange={function(e){var v=e.target.value;var u=Object.assign({},l,{status:v});setLe(function(p){return p.map(function(x){return x.id===l.id?u:x;});});saveLead(u);}} style={{padding:"4px 6px",background:st.c+"10",border:"1px solid "+st.c+"25",borderRadius:7,color:st.c,fontSize:9,fontWeight:600,outline:"none"}}>{LS.map(function(x){return<option key={x.v} value={x.v}>{x.i}{x.l}</option>;})}</select><button onClick={function(){setLe(function(p){return p.filter(function(x){return x.id!==l.id;});});deleteLead(l.id);}} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:12}}>🗑️</button></div>;});})()}</div>
  </div>}
