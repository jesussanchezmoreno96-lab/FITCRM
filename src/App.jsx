...
export default function App(){
  ...
  /* ═══ HOME SCREEN ═══ */
  if(sec==="home") return(
    <div style={{minHeight:"100vh",background:"#0a0d14",color:"#e2e8f0",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}><div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>⚡</div><h1 style={{margin:0,fontSize:34,fontWeight:900}}>FitCRM</h1></div>
      <p style={{color:"#64748b",fontSize:13,marginBottom:40}}>Gestión integral del centro</p>
      <div style={{display:"flex",gap:20,flexWrap:"wrap",justifyContent:"center",maxWidth:900}}>
        {/* Entrenamiento */}
        <button onClick={function(){setSec("entrenamiento");setMv("clientes");}} style={{width:260,padding:"40px 30px",background:"linear-gradient(135deg,#1a1040,#141820)",border:"2px solid #6366f130",borderRadius:20,cursor:"pointer",textAlign:"center"}}>
          <div style={{fontSize:48,marginBottom:12}}>🏋️</div>
          <div style={{fontSize:20,fontWeight:800,color:"#e2e8f0",marginBottom:6}}>Entrenamiento</div>
          <div style={{fontSize:12,color:"#64748b"}}>Clientes, fichas, seguimiento y leads</div>
          <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:14}}>
            <span style={{fontSize:10,padding:"3px 8px",borderRadius:6,background:"#22c55e15",color:"#22c55e",fontWeight:600}}>{cn.a} activos</span>
            {pc>0&&<span style={{fontSize:10,padding:"3px 8px",borderRadius:6,background:"#ef444415",color:"#ef4444",fontWeight:600}}>{pc} avisos</span>}
          </div>
        </button>
        {/* Nutrición */}
        <button onClick={function(){setSec("nutricion");}} style={{width:260,padding:"40px 30px",background:"linear-gradient(135deg,#0a1a10,#141820)",border:"2px solid #22c55e20",borderRadius:20,cursor:"pointer",textAlign:"center"}}>
          <div style={{fontSize:48,marginBottom:12}}>🥗</div>
          <div style={{fontSize:20,fontWeight:800,color:"#e2e8f0",marginBottom:6}}>Nutrición</div>
          <div style={{fontSize:12,color:"#64748b"}}>Próximamente</div>
        </button>
        {/* Fisioterapia */}
        <button onClick={function(){setSec("fisio");}} style={{width:260,padding:"40px 30px",background:"linear-gradient(135deg,#1a0a1a,#141820)",border:"2px solid #a78bfa20",borderRadius:20,cursor:"pointer",textAlign:"center"}}>
          <div style={{fontSize:48,marginBottom:12}}>🩺</div>
          <div style={{fontSize:20,fontWeight:800,color:"#e2e8f0",marginBottom:6}}>Fisioterapia</div>
          <div style={{fontSize:12,color:"#64748b"}}>Reportes y valoraciones</div>
  ...
  if(sec==="nutricion") return(
    <div style={{minHeight:"100vh",background:"#0a0d14",color:"#e2e8f0",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{background:"linear-gradient(135deg,#0a1a10,#0a0d14)",borderBottom:"1px solid #1e2330",padding:"16px 20px"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",gap:12}}>
          <button onClick={function(){setSec("home");}} style={{padding:"6px 14px",background:"#1e2330",border:"1px solid #2a2f3a",borderRadius:8,color:"#94a3b8",fontSize:12,fontWeight:600,cursor:"pointer"}}>← Inicio</button>
  ...
  if(sec==="fisio") return(
    <div style={{minHeight:"100vh",background:"#0a0d14",color:"#e2e8f0",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{background:"linear-gradient(135deg,#1a0a1a,#0a0d14)",borderBottom:"1px solid #1e2330",padding:"16px 20px"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:20}}>
        ...
        {fisioAlerts.length>0&&<div style={{background:"rgba(167,139,250,.06)",border:"1px solid rgba(167,139,250,.2)",borderRadius:14,padding:"14px 20px",marginBottom:20}}>
          ...
          {fisioAlerts.map(function(f){return<div key={f.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",fontSize:12}}>
            ...
            <span style={{color:"#94a3b8"}}>— {f.zones}</span>
        ...
        <div style={B}>{(function(){
          ...
          return ls.map(function(f){
            ...
            return<div key={f.id} style={{padding:"14px 18px",borderBottom:"1px solid #1e2330",background:isAlert?"rgba(167,139,250,.04)":"transparent",opacity:f.done?0.45:1}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                <button onClick={function(){var u=Object.assign({},f,{done:!f.done});setFis(function(p){return p.map(function(x){return x.id===f.id?u:x;});});saveFisio(u);}} style={{width:20,height:20,borderRadius:6,border:f.done?"2px solid #22c55e":"2px solid #2a2f3a",background:f.done?"#22c55e":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:10,color:"#fff"}}>{f.done?"✓":""}</button>
              ...
              {f.notes&&<div style={{fontSize:11,color:"#94a3b8",marginBottom:3}}>📋 Valoración: {f.notes}</div>}
  ...
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:56}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      <button onClick={function(){setSec("home");}} style={{padding:"6px 12px",background:"#1e2330",border:"1px solid #2a2f3a",borderRadius:8,color:"#94a3b8",fontSize:11,fontWeight:600,cursor:"pointer"}}>← Inicio</button>
      ...
      <span style={{fontSize:19,fontWeight:800}}>Entrenamiento</span>
    </div>
  ...
  <div style={{display:"flex",gap:4,padding:"6px 0"}}>{[["clientes","📊","Fichas"],["panel","👥","Clientes"],["seguimiento","📋","Seguimiento"],["leads","🎯","Leads"]].map(function(x){var active=mv===x[0];return<button key={x[0]} onClick={function(){setMv(x[0]);}} style={{padding:"12px 20px",border:"none",borderBottom:active?"3px solid #6366f1":"3px solid transparent",background:active?"rgba(99,102,241,.1)":"transparent",color:active?"#e2e8f0":"#64748b",fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:8,borderRadius:"8px 8px 0 0"}}><span style={{fontSize:18}}>{x[1]}</span>{x[2]}{x[0]==="seguimiento"&&pc>0?<span style={{fontSize:9,padding:"2px 6px",borderRadius:8,background:"#ef4444",color:"#fff",fontWeight:800}}>{pc}</span>:null}</button>;})}</div>
  </div></div>
  ...
  {fisioAlerts.length>0&&mv==="clientes"&&<div style={{background:"rgba(167,139,250,.06)",border:"1px solid rgba(167,139,250,.2)",borderRadius:12,padding:"12px 16px",marginBottom:16}}>
    ...
    {fisioAlerts.slice(0,3).map(function(f){return<div key={f.id} style={{fontSize:11,color:"#94a3b8",marginTop:4}}><b style={{color:"#e2e8f0"}}>{f.client}</b> — {f.plan||f.zones}</div>;})}
  </div>}

  {mv==="panel"&&<div>
    <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:24}}>
      {[["Total",cn.t,"#e2e8f0"],["Activos",cn.a,"#22c55e"],["Pausados",cn.p,"#f59e0b"],["Baja",cn.b,"#ef4444"]].map(function(x){return<div key={x[0]} style={{background:"#141820",borderRadius:14,padding:"22px 26px",border:"1px solid #1e2330",flex:"1 1 120px",minWidth:120}}><div style={{fontSize:34,fontWeight:900,color:x[2]}}>{x[1]}</div><div style={{fontSize:10,color:"#8892a4",marginTop:4,fontWeight:600,textTransform:"uppercase"}}>{x[0]}</div></div>;})}
    </div>
    <div style={{display:"flex",gap:6,marginBottom:16}}>{[["todos","Todos",cn.t],["activo","Activos",cn.a],["pausado","Pausados",cn.p],["baja","Baja",cn.b]].map(function(x){return<button key={x[0]} onClick={function(){setFs(x[0]);}} style={{padding:"7px 14px",borderRadius:9,border:fs===x[0]?"1px solid #6366f1":"1px solid #2a2f3a",background:fs===x[0]?"rgba(99,102,241,.1)":"transparent",color:fs===x[0]?"#e2e8f0":"#64748b",fontSize:11,fontWeight:600,cursor:"pointer"}}>{x[1]}({x[2]})</button>;})}</div>
    <div style={B}>{(function(){var ls=cl;if(fs!=="todos")ls=ls.filter(function(c){return c.status===fs;});if(!ls.length)return<div style={{padding:40,textAlign:"center",color:"#475569"}}>Sin clientes</div>;return ls.map(function(c){var s=ST[c.status]||ST.activo;return<div key={c.id} onClick={function(){setSel(c);setTab("perfil");setMv("clientes");}} style={{padding:"16px 20px",borderBottom:"1px solid #1e2330",display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}><div style={{width:36,height:36,borderRadius:9,background:s.c+"15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:s.c}}>{ini(c)}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</div><div style={{fontSize:10,color:"#64748b"}}>{c.objective}</div></div><select value={c.status} onClick={function(e){e.stopPropagation();}} onChange={function(e){var v=e.target.value;sv(function(p){return p.map(function(x){return x.id===c.id?Object.assign({},x,{status:v}):x;});});}} style={{padding:"4px 8px",background:s.c+"10",border:"1px solid "+s.c+"25",borderRadius:7,color:s.c,fontSize:10,fontWeight:600,outline:"none"}}>{Object.entries(ST).map(function(e){return<option key={e[0]} value={e[0]}>{e[1].l}</option>;})}</select><button onClick={function(e){e.stopPropagation();if(confirm("¿Eliminar DEFINITIVAMENTE a "+c.name+"?")){setCl(function(p){return p.filter(function(x){return x.id!==c.id;});});deleteClient(c.id);if(sel&&sel.id===c.id)setSel(null);}}} style={{padding:"6px 10px",background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",borderRadius:7,color:"#ef4444",fontSize:11,cursor:"pointer",fontWeight:600,flexShrink:0}}>🗑️</button></div>;});})()}</div>
  </div>}
  ...
  {mv==="leads"&&<div>
    ...
    <div style={{marginBottom:10}}><div style={{fontSize:10,color:"#8892a4",fontWeight:600,textTransform:"uppercase",marginBottom:6}}>Año</div><div style={{display:"flex",gap:5}}>{(function(){var years=["todos"];le.forEach(function(l){if(l.year&&years.indexOf(l.year)===-1)years.push(l.year);});years.sort();return years.map(function(y){var n=y==="todos"?le.length:le.filter(function(l){return l.year===y;}).length;return<button key={y} onClick={function(){setLyear(y);setLmonth("todos");}} style={{padding:"6px 14px",borderRadius:8,border:lyear===y?"1px solid #6366f1":"1px solid #1e2330",background:lyear===y?"rgba(99,102,241,.1)":"transparent",color:lyear===y?"#e2e8f0":"#64748b",fontSize:11,fontWeight:600,cursor:"pointer"}}>{y==="todos"?"Todos":y} ({n})</button>;});})()}</div></div>
