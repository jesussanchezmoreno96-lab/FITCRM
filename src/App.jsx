...
var LS=[{v:"nuevo",l:"Nuevo",c:"#38bdf8",i:"✨"},{v:"contactado",l:"Contactado",c:"#f59e0b",i:"📞"},{v:"interesado",l:"Interesado",c:"#a78bfa",i:"🔥"},{v:"convertido",l:"Convertido",c:"#22c55e",i:"✅"},{v:"perdido",l:"Perdido",c:"#ef4444",i:"❌"}];
...
export default function App(){
  ...
  var lf_=_({name:"",phone:"",source:"",interest:"",status:"nuevo"}),lf=lf_[0],setLf=lf_[1];
  ...
  {mv==="clientes"&&<div>
    ...
    {sel?<div style={B}>
      ...
      <div style={{padding:20}}>
        ...
        {tab==="ex"&&<div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:14}}>{EX.map(function(x,i){return<button key={i} onClick={function(){setEt(i);}} style={{padding:"5px 10px",borderRadius:7,border:et===i?"1px solid "+EXC[i]:"1px solid #2a2f3a",background:et===i?EXC[i]+"12":"transparent",color:et===i?EXC[i]:"#64748b",fontSize:10,fontWeight:600,cursor:"pointer"}}>{EX[i].split(" ")[0].substr(0,4)}</button>;})}</div>
          <h3 style={{margin:"0 0 10px",fontSize:15,fontWeight:700,color:EXC[et]}}>{EX[et]}</h3>
  ...
  {mv==="leads"&&<div>
    ...
    {/* Leads list */}
    <div style={B}>{(function(){var ls=le.slice();if(lyear!=="todos")ls=ls.filter(function(l){return l.year===lyear;});if(lmonth!=="todos")ls=ls.filter(function(l){return l.month===lmonth;});if(lfl==="activos")ls=ls.filter(function(l){return l.status!=="convertido"&&l.status!=="perdido";});else ls=ls.filter(function(l){return l.status===lfl;});if(!ls.length)return<div style={{padding:40,textAlign:"center",color:"#475569"}}>{le.length===0?"Añade tu primer lead":"Sin leads con estos filtros"}</div>;return ls.map(function(l){var st=LS.find(function(x){return x.v===l.status;})||LS[0];return<div key={l.id} style={{padding:"12px 18px",borderBottom:"1px solid #1e2330",display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18}}>{st.i}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700}}>{l.name}{l.source?<span style={{fontSize:9,padding:"2px 5px",borderRadius:5,background:"#1e2330",color:"#94a3b8",marginLeft:6}}>{l.source}</span>:null}{l.month?<span style={{fontSize:9,padding:"2px 5px",borderRadius:5,background:"#0d1017",color:"#64748b",marginLeft:4}}>{l.month} {l.year}</span>:null}</div>{l.phone&&<div style={{fontSize:10,color:"#64748b"}}>{l.phone}</div>}{l.notes&&<div style={{fontSize:10,color:"#475569",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:400}}>{l.notes}</div>}</div><select value={l.status} onChange={function(e){var v=e.target.value;var u=Object.assign({},l,{status:v});setLe(function(p){return p.map(function(x){return x.id===l.id?u:x;});});saveLead(u);}} style={{padding:"4px 6px",background:st.c+"10",border:"1px solid "+st.c+"25",borderRadius:7,color:st.c,fontSize:9,fontWeight:600,outline:"none"}}>{LS.map(function(x){return<option key={x.v} value={x.v}>{x.i}{x.l}</option>;})}</select><button onClick={function(){setLe(function(p){return p.filter(function(x){return x.id!==l.id;});});deleteLead(l.id);}} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:12}}>🗑️</button></div>;});})()}</div>
  </div>}
